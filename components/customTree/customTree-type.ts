...
interface CustomTreeRef {
  expandAllNode: () => Promise<{ keys: Key[]; nodes: any[] }>
  closedAllNode: () => Promise<{ keys: Key[]; nodes: any[] }>
  getAllExpandNodes: <T extends 'keys' | 'nodes'>(target: T) => T extends 'keys' ? string[] : any[]
  checkedAllNode: () => Promise<{ keys: Key[]; nodes: any[] }>
  disCheckedAllNode: () => Promise<{ keys: Key[]; nodes: any[] }>
  scrollTo: (key: Key, keepExpand: boolean, offset?: number,) => null
  getAllCheckedNode: () => Promise<{ keys: Key[]; nodes: any[] }>
}
